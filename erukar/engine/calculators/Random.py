...
class Random:
    ...
    def create_random_distribution(possibilities, weights, min_weight=-0.3):
        bins = Random.calculate_bin_widths(weights, min_weight)
        values = np.array(possibilities)
        return Random.order_bins_and_values(bins, values)

    def calculate_bin_widths(W, min_weight=-0.3):
        '''Determines the probablistic proportions for binning'''
        if all(w_i <= min_weight for w_i in W) or len(set(W)) == 1:
            return [1/len(W)] * len(W)

        W_prime = [w_i for w_i in W if w_i > min_weight]
