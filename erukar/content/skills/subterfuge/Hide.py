...
class Hide(SidebarAbility):
    Name = 'Hide'
    HideSuccess = 'You attempt to hide!'
    CurrentLevel = ''
    NextLevel = ''
    ActiveAbbreviation = 'HIDE'
    ...
    def total_evasion(self, ap):
        return Hide.evasion_base(self.level)\
                + ap * Hide.evasion_per_ap(self.level)

    def current_level_description(self):
        evd_base = Hide.evasion_base(self.level) * 100.0
        evd_ap = Hide.evasion_per_ap(self.level) * 100.0
        return self.CurrentLevel.format(evd_base, evd_ap)

    def next_level_description(self):
        evd_base = Hide.evasion_base(self.level + 1) * 100.0
        evd_ap = Hide.evasion_per_ap(self.level + 1) * 100.0
        return self.CurrentLevel.format(evd_base, evd_ap)
    ...
    def perform(self, cmd):
        ...
        msg = self.HideSuccess.format(bonus * 100.0, player.evasion())
