from erukar.system.engine import Manager, PlayerNode, Enemy, Dead
...
class TurnManager(Manager):
    ...
    def subscribe(self, player):
        ...
        if self.active_player is None or (isinstance(player, PlayerNode) and isinstance(self.active_player, Enemy)):
    ...
    def is_playable(self, p):
        if isinstance(p, PlayerNode):
            return not p.has_condition(Dead)
        return False
